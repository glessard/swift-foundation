...
internal class JSONMap {
    ...
    func copyInBuffer() {
        let _: Void = dataLock.withLock { (state: inout (start: UnsafePointer<UInt8>, count: Int, allocation: UnsafePointer<UInt8>?)) -> Void in
            ...
            let span = Span<UInt8>(unsafeStart: state.start, count: state.count, owner: state)
    ...
    func withSpan<T>(
        ...
        try dataLock.withLock {
            let span = Span<UInt8>(unsafeStart: $0.start, count: $0.count, owner: $0.start)
...
extension JSONScanner {
    ...
    private static func parseEscapeSequence(
      ...
      precondition(!jsonBytes.isEmpty, "Scanning should have ensured that all escape sequences are valid shape")
        ...
        case UInt8(ascii:"u"):
            return try parseUnicodeSequence(from: jsonBytes.extracting(unchecked: 1..<jsonBytes.count), into: &string, fullSource: fullSource)
    ...
    internal static func parseUnicodeSequence(
        ...
        if UTF16.isLeadSurrogate(bitPattern) {
            ...
            var trailingBytes = jsonBytes.extracting(droppingFirst: index1)
            guard trailingBytes.count >= 2 else {
                throw JSONError.expectedLowSurrogateUTF8SequenceAfterHighSurrogate(location: .sourceLocation(at: index1, of: jsonBytes, in: fullSource))
            }
...
//        }
        ...
        static func sourceLocation(
            ...
            var line = 1
            var col = 0
            let end = min(location.advanced(by: 1), fullSource.count)
            while index < end {
                switch fullSource[unchecked: index] {
                case ._return:
                    let next = index + 1
                    if next <= location, fullSource[next] == ._newline {
                        index = next
