...
extension JSON5Scanner {
    ...
    struct DocumentReader: ~Escapable {
        ...
        @inline(__always)
        func byteOffset(at index: Int) -> Int {
            index
        }
        ...
        func peekU32() throws -> (scalar: UnicodeScalar, length: Int)? {
            ...
            if firstChar == ._backslash {
                ...
                case UInt8(ascii: "u"):
                    ...
                    let remaining = bytes.extracting(droppingFirst: 2) // Skip \u
                ...
                case UInt8(ascii: "x"):
                    ...
                    let remaining = bytes.extracting(droppingFirst: 2) // Skip \x
...
extension JSON5Scanner {
    ...
    static func stringValue(
        ...
        guard var output = String._tryFromUTF8(jsonBytes.extracting(unchecked: jsonBytes.startIndex..<index)) else {
    ...
    static func _slowpath_stringValue(
        ...
        while index < jsonBytes.endIndex {
            ...
            case ._backslash:
                ...
                index = try parseEscapeSequence(from: jsonBytes.extracting(droppingFirst: index+1), into: &output, fullSource: fullSource)
                chunkStart = index
    ...
    private static func parseEscapeSequence(
        ...
        case UInt8(ascii:"u"):
            return try JSONScanner.parseUnicodeSequence(from: jsonBytes.extracting(droppingFirst: 1), into: &string, fullSource: fullSource, allowNulls: false)
        case UInt8(ascii:"x"):
        ...
        case let ascii: // default
            throw JSONError.unexpectedEscapedCharacter(ascii: ascii, location: .sourceLocation(at: index, of: jsonBytes, in: fullSource))
        }
        return index+1
    }
    ...
    static func validateLeadingDecimal(
      from jsonBytes: Span<UInt8>, fullSource: Span<UInt8>
        ...
        guard !jsonBytes.isEmpty else {
            throw JSONError.unexpectedCharacter(ascii: UInt8(ascii: "."), location: .sourceLocation(at: 0, of: jsonBytes, in: fullSource))
        }
